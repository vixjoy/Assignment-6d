  ...
  function($scope, $location, $stateParams, $state, Listings){
    ...
    $scope.remove = function() {
       ...
       */
       ...
       Listings.delete(listingId).then(function(response){
    ...
    //related to the function in the view for map listings
    $scope.showMap = function(){
        ...
        Listings.getAll().then(function(response){
          ...
          for(var i = 0; i < response.data.length;i++){
            if(response.data[i].coordinates){
              $scope.mListings.push(response.data[i]);
            }
          }
        }, function(error){
          $scope.loading = false;
    ...
    /* Map properties */ //allows access to zoom and center in the map view
    $scope.map = {
      center: {
